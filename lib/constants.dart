...
double mushafFontSize=40.0;

int bookMarkedSura=1;
int bookMarkedAyah=1;

Uri quranAppUrl=Uri.parse('https://github.com/mohamedabdallatif/quran');
...
Future saveBookMark (sura,ayah)async{
    final prefc=await SharedPreferences.getInstance();
    await prefc.setInt('sura', sura);
    await prefc.setInt('ayah', ayah);
  }
  Future readBookMark()async{
    final prefc=await SharedPreferences.getInstance();
    try{
    bookMarkedSura =  prefc.getInt('sura')!;
    bookMarkedAyah =  prefc.getInt('ayah')!;
    return true;
    }
    catch(_){
      print('error');
...
Future readjson()async{
  ...
  arabic=data['quran'];
  malaylam=data['malayalam'];
  return quran=[arabic,malaylam];
}
List <Map>arabicName = [
